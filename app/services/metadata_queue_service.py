...
class MetadataQueueService:
    ...
    def __init__(self, db, provider: MetadataProvider, matcher: GameMatchingService, artwork: ArtworkService | None = None, interval=.3):
        ...
        for local in self.db.queued_games():
            ...
            try:
                ...
                if local["external_game_id"] and local["metadata_source"] == self.provider.name:
                    ...
                    self.db.apply_metadata(game_id, external, self.provider.name, score, "refresh", MetadataStatus.MATCHED)
                    state = "matched"
                else:
                    ...
                    if state == "matched": self.db.apply_metadata(game_id, ranked[0].game, self.provider.name, ranked[0].score, "automatic", MetadataStatus.MATCHED); external = ranked[0].game
                    else: self.db.set_metadata_status(game_id, MetadataStatus(state)); external = None
                if state == "matched" and self.artwork and external:
                    try:
                        cover = self.artwork.fetch(game_id, external)
                        if cover: self.db.set_cover(game_id, str(cover), self.artwork.provider.name)
                    except Exception: LOGGER.exception("Cover-Fehler für Game %s", game_id)
                stats[state] += 1
