...
Cover werden validiert (JPEG, PNG oder WebP), zunächst temporär geschrieben und atomar unter `data/covers/<provider>_<external-id>_<game-id>.<ext>` ersetzt. Ein explizites **Cover aktualisieren** umgeht den Cache, lässt bei Fehlern aber das bisherige Cover unangetastet; alte Dateien werden erst nach erfolgreichem Austausch und nur innerhalb des Covercache entfernt. Über **Cover auswählen...** können SteamGridDB-Kandidaten asynchron angesehen und manuell bestätigt werden. Die Cover-Queue zeigt Fortschritt und kann pausiert, fortgesetzt oder abgebrochen werden. Coverfehler verändern einen erfolgreichen Metadatenmatch nicht. Ohne Netzwerk oder Credentials bleiben Scan, Cleanup, Tabelle, bestehende Cover und die gesamte lokale Bibliothek verfügbar; lediglich Queue-Einträge erhalten einen verständlichen Fehlerstatus. HTTP-Anfragen haben Timeouts, begrenzte Retries und Frequenz, beachten `Retry-After` bei 429 und erzeugen keine parallele Anfrageflut.
